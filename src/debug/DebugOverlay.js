...
import { geometryContainsPoint } from '../quiz/targetGeometry.js';
...
const CITY_COLOR = 0x2f6b3d;
const CITY_ALPHA = 0.65;

function drawDimmedNonQuizPolygons(graphics, geometry, quizCountryTargets, projectFn) {
  const isQuizCountry = quizCountryTargets.some((target) =>
    geometryContainsPoint(geometry, target.lon, target.lat));
  if (isQuizCountry) {
    return;
  }
